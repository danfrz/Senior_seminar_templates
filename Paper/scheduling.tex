...
When two contexts have read and write access to the same portion of the memory, any read or write to that memory must be made synchronous to avoid race conditions. A race condition is a timing dependent error where two threads updating the same memory at the same time overwrite eachother in a way that might cause the threads to function incorrectly.~\cite{Systems} A system can be made synchronous by employing \emph{locks}. Principles of Computer System Design by Saltzer and Kaashoek define locks as ``A flag associated with a data object, set by a thread to warn concurrent threads that the object is in use and that it may be a mistake for other threads to read or write it.'' ~\cite{Systems}
See figure \ref{fig:locks} for an example employing synchronization. The \emph{synchronized} keyword in java implicitly employs locks so that no reads or writes can be made to the object that was synchronized until the first synchronized block ends. \todo[inline]{In this code, self.wait() happens within the synchronized block, so wouldn't processing still be "inside" the synchronized wait block while notify needed to happen in another synchronized block? I need to look into synchronized further. I think I should include an example with less ceremony: a non-graphical program.}
