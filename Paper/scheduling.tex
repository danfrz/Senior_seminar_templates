...
\Indp\Indpp
	...
	\ForAll{sd in sched\_domains of cur\_cpu}{
		\If{sd has idle cores}{first\_cpu = $1^{st}$ idle CPU of sd}
		\Else{
		...
		busiest = overloaded sg with the highest load \\
			\,($\textbf{or}$, if inexistent) imbalanced sg with the highest load \\
			\,($\textbf{or}$, if inexistent) sg with the highest load \\
...
\todo[inline]{(LaTeX) the forall should start at line 1...}
\end{algorithm}
}
